...
function GnRadaptation(percent_pressure,percent_flow,filename)
comp_t=250;
delta_P=1+percent_pressure;
delta_Q=1+percent_flow;
K_c1=1;
K_c2=1;
K_m1=1;
K_m2=1;
...
% Input parameters that the program takes in
...
%t_stop = [1, 9, 16, 23, 30, 37, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000];
t_stop = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000];
format short e

% Transmural pressure
P_h = 93.0*133.32237*10.0;  %(g/cm s^2)
...
Lambda_M = 1.1; % was 1.1
Lambda_0 = 0.4; % was 0.4
T_M = 150.0*1000.0*10.0;
C_basal = 0.68;
...
sigma_act_h = T_M*(1.0 - exp(-C_basal^2))*(1.0 - ((Lambda_M - 1.0)/(Lambda_M - Lambda_0))^2);
...
c_e = sigma_eh/(G_et^2 - (G_et^2*G_ez^2)^(-1));
c_m2 = (sigma_mh - sigma_act_h)/(G_mh^2*(G_mh^2 - 1.0)*exp(c_m3*(G_mh^2 - 1.0)^2));

str_ch = 0.0;
for i = 1:4
    str_ch = str_ch + c_frac(i)*G_ch^2*(G_ch^2 - 1.0)*...
        exp(c_c3*(G_ch^2 - 1.0)^2)*(sin(alpha_ckh(i)))^2;
end
c_c2 = sigma_ch/str_ch;         %(g/cm.s^2)

%-----------------------------------------------
% Mechanical parameters for compressive behavior
%-----------------------------------------------
c_c2_c = c_m2;
c_c3_c = c_m3;
c_m2_c = c_m2;
...
dwdLt_c = 0.0;
for i = 1:4
    dwdLt_c = dwdLt_c + phi_ck(i)*h_h*dWkdLn(G_ch, c_c2, c_c3, c_c2_c, c_c3_c)*G_ch*(sin(alpha_ckh(i)))^2;
end
...
kq_m =0.04953;  % for muscle
kq_c =0.34687;   % for collagen
...
%                       cos(alpha_ckh(i)))^2/Lc_k(i)^3;
                    Lc_k(i)
                    dWck_dLn(i) = dWkdLn(Lc_k(i)*G_ch, c_c2, c_c3, c_c2_c, c_c3_c);
                    ...
                    ddwddLt_c   = ddwddLt_c + (M_ckh(i)/rho_s)*sQ_ck*...
                        (ddWkddLn(Lc_k(i)*G_ch, c_c2, c_c3, c_c2_c, c_c3_c)*(dLndLt)^2 + ...
                        dWck_dLn(i)*ddLnddLt);
...
%                           cos(alpha_tau(i)))^2/Lc_k(i)^3;
                        ...
                        dWck_dLn(i) = dWkdLn(Lc_kn(i),c_c2,c_c3,c_c2_c,c_c3_c);
                        ...
                        ddwddLt_c   = ddwddLt_c + (mc_tau/rho_s)*sq_ck* ...
                            (ddWkddLn(Lc_kn(i), c_c2, c_c3, c_c2_c, c_c3_c)*(dLndLt)^2 + ...
...
%           ddwdLtdLz_e = (M_e/rho_s)*ddWeddLn(L_t*G_et, L_z*G_ez, 3, c_e)*G_et*G_ez;
        ...
        m_c = [rm_c_dtauversion(1, m_basal_ck(1), nc_stress(1), dn_C, K_c1, K_c2),...
            rm_c_dtauversion(1, m_basal_ck(2), nc_stress(2), dn_C, K_c1, K_c2),...
            rm_c_dtauversion(1, m_basal_ck(3), nc_stress(3), dn_C, K_c1, K_c2), ...
            rm_c_dtauversion(1, m_basal_ck(4), nc_stress(4), dn_C, K_c1, K_c2)];
        m_m = rm_m_dtauversion(1, m_basal_m, nm_stress, dn_C, K_m1, K_m2);
...
%              C_t, C_basal, T_S, tau_w, tau_wh)
    ...
    %6-10
    fprintf(fid,    '%10.8f\t   %10.8f\t    %10.8f\t                %10.8f\t                %10.8f\t',...
                    Q_M,        C_t,        m_c(1)/m_basal_ck(1),   m_c(2)/m_basal_ck(2),   m_c(3)/m_basal_ck(3)  );
    %11-15
    fprintf(fid,    '%10.8f\t               %10.8f\t        %10.8f\t        %10.8f\t        %10.8f\t',...
                    m_c(4)/m_basal_ck(4),	m_m/m_basal_m,  tau_w/tau_wh,   nc_stress(1),   nc_stress(2) );
    %16-20
    fprintf(fid,    '%10.8f\t       %10.8f\t        %10.8f\t       %10.8f\t    %10.8f\t',...
                    nc_stress(3),   nc_stress(4),   nm_stress,     M_m,        M_e );
    %21-25
    fprintf(fid,    '%10.8f\t       %10.8f\t        %10.8f\t       %10.8f\t    %10.8f\t',...
                    M_ck(1),        M_ck(2),        M_ck(3),       M_ck(4),    T_act );
    %26-30
    fprintf(fid,    '%10.8f\t       %10.8f\t        %10.8f\t       %10.8f\t       %10.8f\t',...
                    m_basal_ck(1),  m_basal_ck(2),  m_basal_ck(3), m_basal_ck(4),  m_basal_m);
    %31-32
...
%===========================
% BEGIN Pressure-Radius Curves
%===========================    

    % pressure-inner radius data (passive tone)
    [flg_ts, flg_n] = check_stop(t, t_stop, dt);
...
%       invivo_radius = a_t;
        ...
        % Pressure-dimater output file
        str_1 = sprintf('_time%i', t_stop(flg_n));
        str_2 = strcat(appended, str_1);
        fid2  = fopen(str_2, 'w');

        P_passive = 140.0;               % mmHg
...
%                   Lc_kn = Lc_k(i)*G_ch;
                    ...
                    dwdLt_c = dwdLt_c + (M_ckh(i)/rho_s)*sQ_ck*...
                        dWkdLn(Lc_k(i)*G_ch, c_c2, c_c3, c_c2_c, c_c3_c)*dLndLt;
                    dwdLz_c = dwdLz_c + (M_ckh(i)/rho_s)*sQ_ck*...
                        dWkdLn(Lc_k(i)*G_ch, c_c2, c_c3, c_c2_c, c_c3_c)*dLndLz;
                end
                % Smooth Muscle presented at time zero
                Lm_n = L_t*G_mh;
                sQ_m = DQ_m(n_t)*DQ2_m(1);

                dwdLt_m = dwdLt_m + (M_mh/rho_s)*sQ_m*dWmdLn(Lm_n, c_m2, c_m3, c_m2_c, c_m3_c)*G_mh;
            end
            ...
            for n_tau = tn0:n_t
                ...
                Lc_k  = sqrt(L_t^2*sin(alpha_tau).^2 + L_z^2*cos(alpha_tau).^2);
                Lc_kn = G_ch*Lc_k./Lc_k_tau;
                invivo_Lc_k = sqrt(invivo_L_t^2*sin(alpha_tau).^2 + L_z^2*cos(alpha_tau).^2);
                invivo_Lc_kn = G_ch*invivo_Lc_k./Lc_k_tau;

                for i = 1:4
                    mc_tau = Dm_c(n_tau, i);
                    % If the stretch is larger than yiedling value,
                    % then it degradated. so it will not included sin mass

                    if (invivo_Lc_kn(i) <= y_Lkn)
                        sq_ck  = q_i((n_t - n_tau)*dt, kq_c)*DQ2_c(n_tau, i);
                        dLndLt = (G_ch/Lc_k_tau(i))*L_t*sin(alpha_tau(i))^2/Lc_k(i);
                        dLndLz = (G_ch/Lc_k_tau(i))*L_z*cos(alpha_tau(i))^2/Lc_k(i);

                        dwdLt_c = dwdLt_c + (mc_tau/rho_s)*sq_ck*...
                            dWkdLn(Lc_kn(i), c_c2, c_c3, c_c2_c, c_c3_c)*dLndLt*wt;
                        dwdLz_c = dwdLz_c + (mc_tau/rho_s)*sq_ck*...
                            dWkdLn(Lc_kn(i), c_c2, c_c3, c_c2_c, c_c3_c)*dLndLz*wt;
                    end
                end
                mm_tau = Dm_m(n_tau);
                Lm_n = G_mh*L_t/Lt_tau;
                invivo_Lm_n = G_mh*invivo_L_t/Lt_tau;

                if (invivo_Lm_n <= y_Lmn)
                    sq_m    = q_i((n_t - n_tau)*dt, kq_m)*DQ2_m(n_tau);
                    dwdLt_m = dwdLt_m + (mm_tau/rho_s)*sq_m*...
                        dWmdLn(Lm_n, c_m2, c_m3, c_m2_c, c_m3_c)*(G_mh/Lt_tau)*wt;
                end
            end
...
%           dwdLz_e = (M_e/rho_s)*dWedLn(L_t*G_et, L_z*G_ez, 2, c_e)*G_ez;
            ...
            dwdLt = dwdLt_c + dwdLt_m + dwdLt_e;
%           dwdLz = dwdLz_c + dwdLz_m + dwdLz_e;

            P_passive = (1.0/L_z)*dwdLt/radius;

            fprintf(fid2, '%10.8f\t %10.8f\n', radius, P_passive/(133.32237*10.0));
            radius = radius - dr;
        end
...
%===========================
    ...
    for n_tau = tn0:n_t
        ...
        for i = 1:4
            ...
            beta_c          = dWkdLn(Lc_kn(i), c_c2, c_c3, c_c2_c, c_c3_c)/dWcdLn_h;
...
fclose(fid);
timeVector      = (0 : num_t-1) * dt;    % in days
normalizedRad   = Da / a_M;             % a(t)/a_M

figure;
semilogx(timeVector, normalizedRad, 'LineWidth', 2);
xlabel('Time (days)');
ylabel('Normalized Radius a(t) / a_M');
title('Normalized Radius Evolution Over Time');
grid off;
%clear *
